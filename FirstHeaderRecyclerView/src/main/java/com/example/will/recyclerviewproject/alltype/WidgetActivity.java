...
//        通过设置LineLayoutManager实现横向或者是竖向的listview.
        LinearLayoutManager layoutManager = new LinearLayoutManager(this);
        layoutManager.setOrientation(isVer ? LinearLayoutManager.VERTICAL : LinearLayoutManager.HORIZONTAL);
        recyclerView.setLayoutManager(layoutManager);
        ListAdapter listAdapter = new ListAdapter(this, isVer);
        recyclerView.setAdapter(listAdapter);
        listAdapter.setOnItenClickListener(new ListAdapter.OnItenClickListener() {
            @Override
            public void onRecyclerViewItenClick(int position) {
                Toast.makeText(WidgetActivity.this, "第:" + position + "被点击了", Toast.LENGTH_SHORT).show();
            }
        });
